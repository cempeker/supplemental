...
calculateOptimalExtremAdj = function(dat){
  ...
  for(i in 1:nrow(dat)){
    ...
    if(dat[i,'exp_cond'] == 'crowd10'){
      dat[i,'opt_report'] = (-4)*dat[i,'xi_shared'] + 5*dat[i,'xi_private']
    } else if(dat[i,'exp_cond'] == 'contest10'){
      dat[i,'opt_report'] = (1/11)*dat[i,'xi_shared'] + (10/11)*dat[i,'xi_private']
    } else {
...
#### generate non-expert plot (Figure C2)
plt_nonexpert = plotNonExp(dat_nonexp)
pdf(file = "./s2_nonexpert.pdf",width=6,height=3.5)
plt_nonexpert
dev.off()

#### Generate percentage extremization plot (Figure 5)
plt_extremperc = plotExtremPerc(dat)
pdf(file = "./s2_extremperc.pdf",width=6.4,height=3.8)
plt_extremperc
dev.off()

#### Plot Average extremizing adjustment across individuals (Appendix)
plt_extremind = plotExtremAdjInd(dat)
pdf(file = "./s2_extremind.pdf",width=7,height=5)
plt_extremind
dev.off()
